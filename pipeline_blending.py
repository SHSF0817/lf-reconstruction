...
def warp_mpis(mpis, source_uv, target_uv, disp_range):
    ...
    ### render disp maps
    N, D, H, W, _ = mpis.get_shape().as_list()
    disp_buffer = tf.linspace(disp_range[0,0], disp_range[0,1], D) # shape [D]
    disp_buffer = tf.reshape(disp_buffer, [1, D, 1, 1, 1])

    ### depth layers
    disp_maps = weights * disp_buffer # shape [N, D, H, W, 1]

    ### render refine net input
    bg_color = tf.cumsum(weights * mpis_color, axis=1)
    bg_color = tf.transpose(bg_color, [1,2,3,0,4])
    bg_color = tf.reshape(bg_color, [1, D, H, W, N*3])

    ### fuse four mpis
    color_layer = tf.transpose(weights * mpis_color, [1,2,3,0,4]) # [D, H, W, N, 3]
    color_layer = tf.reshape(color_layer, [1, D, H, W, N*3]) # [1, D, H, W, 3*N]

    return rendered_views, occ_map, disp_maps, bg_color, weights * mpis_color

def render_views(mpi, disp_range):
    # back to front compositing
    mpi_color = mpi[..., 0:3]  # [N, D, H, W, 3]
    mpi_alpha = mpi[..., 3:4]  # [N, D, H, W, 1]

    weights = mpi_alpha * tf.cumprod(1. - mpi_alpha + 1e-10, axis=1, exclusive=True, reverse=True) # [N, D, H, W, 1]
    rendered_view = tf.reduce_sum(weights * mpi_color, axis=1) # [N, H, W, 3]

    # render disp maps
    N, D, H, W, _ = mpi.get_shape().as_list()
    disp_buffer = tf.linspace(disp_range[0,0], disp_range[0,1], D) # shape [D]
    disp_buffer = tf.reshape(disp_buffer, [1, D, 1, 1, 1])
    disp_map = tf.reduce_sum(weights * disp_buffer, axis=1) # shape [N, H, W, 1]

    return rendered_view, disp_map, weights * mpi_color
...
def views_fusion(rendered_views, occ_map, disp_maps, source_uv, target_uv, disp_range):
    ...
    final_view = tf.reduce_sum(rendered_views*weights*occ_map, axis=0, keepdims=True) / (tf.reduce_sum(occ_map*weights, 0, keepdims=True) + 1e-10) # [1, H, W, 3]
...
def render_occ_mask(rendered_views):
    N, H, W, _ = rendered_views.get_shape().as_list()
    rendered_views = tf.transpose(rendered_views, [1,2,0,3]) # shape [H, W, N, 3]
    net_input = tf.reshape(rendered_views, [1, H, W, N*3])
    occ_masks = tf.nn.sigmoid(occ_mask_net(net_input, N))
    # occ_masks = tf.nn.softmax(occ_mask_net(net_input, N))

    occ_masks = tf.transpose(occ_masks, [3, 1, 2, 0])
    return occ_masks
...
class Pipeline():
    ...
    def inference(self):
        ...
        focal_stacks = tf.concat([fs_tl, fs_tr, fs_bl, fs_br], 0) # shape [4, D, H, W, 3]
        rendered_views, occ_map, disp_maps, bg_mpis, mpis_layer = warp_mpis(mpis, self.source_uv, self.target_uv, self.disp_range) # shape [4, H, W, 3]
       
        refine_input = bg_mpis
        refine_mpi, mpi_weights = blending_net(refine_input)
        final_view, disp_map, mpi_layer = render_views(refine_mpi, self.disp_range)

        if self.train == True:
            return final_view, disp_map
        else:
            return final_view, refine_mpi, bg_mpis, disp_maps, rendered_views, mpis, focal_stacks, mpi_weights, mpis_layer, mpi_layer
    ...
    def per_l1_loss(self, real, fake):
        weights = [11,0.385,0.208,0.270,0.179,6.667]
    ...
    def loss(self, pre_view): 
        lp_l1 = self.per_l1_loss(self.target_imgs, pre_view)
        # lf = self.focal_loss(self.source_imgs, focal_imgs)
        # lg = self.gradient_loss(self.target_imgs, pre_view)
        loss_all = lp_l1
        return loss_all

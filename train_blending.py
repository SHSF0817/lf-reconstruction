...
flags = tf.app.flags
flags.DEFINE_string('pre_train_model', 'model/pre_generation/model', '')
flags.DEFINE_string('vgg_model', './models/vgg19_model/vgg_19.ckpt', '')
...
flags.DEFINE_integer('max_step', 100000, 'the epochs of training')
...
flags.DEFINE_string('dataset_name','HCI','Inria or Inria')
flags.DEFINE_float('resize_scale', 1.0, 'resize the test images')
flags.DEFINE_string('results_path','results/','the path of output results')
...
def random_views(input_lf, train=True):
    ...
    if train:
        ...
        target_row = np.random.randint(tl_row, tl_row + offset + 1)
        target_col = np.random.randint(tl_col, tl_col + offset + 1)
    else:
        offset = 8
        [tl_row, tl_col] = [0, 0]
...
def train():
    ...
    final_view_train, _ = pipeline_train.inference()
    ...
    variables_mpi= slim.get_variables(scope='mpi_pre_net')
    saver_init = tf.train.Saver(variables_mpi, max_to_keep=1)

    variables_vgg = slim.get_variables(scope='vgg_19')
    saver_vgg = tf.train.Saver(variables_vgg, max_to_keep=1)

    variables_refine = slim.get_variables(scope='refine_net')
    saver_refine = tf.train.Saver(variables_refine)

    variables_all = slim.get_variables()
    saver_all = tf.train.Saver(variables_all)
    ...
    with tf.Session(config = config) as sess:
        ...
        if FLAGS.continue_train == True:
            ...
            saver_refine.restore(sess, FLAGS.model_path + FLAGS.model_name)
            saver_init.restore(sess, FLAGS.pre_train_model)
        else:
            print('--- Initialization......')
            learning_curve = []

            print('--- Loading pretrain weights...')
            saver_vgg.restore(sess, FLAGS.vgg_model)
            saver_init.restore(sess, FLAGS.pre_train_model)
        ...
        while step < FLAGS.max_step:
            ...
            if epoch % 10 == 0:
                ...
                if valid_loss < min_loss:
                    min_loss = valid_loss
                    print('--- Saving model ---')
                    saver_all.save(sess, FLAGS.model_path + FLAGS.model_name, step)

                results = [epoch, mean_ssim, mean_psnr, mean_loss]
                with open('ours_16.csv','a',newline='') as f:
